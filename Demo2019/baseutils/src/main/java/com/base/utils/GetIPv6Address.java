...
public class GetIPv6Address {
    ...
    private final String TAG = "GET_IPV6"; // TAG标签

    private final String getIpv6Cmd = "/system/bin/ip -6 addr show "; // 默认获取Ipv6地址的shell命令
    private final int IPV6LEN_LEFT_BIT = 6; // 默认，截取的Ipv6字符串左起始位
    private final int IPV6LEN_RIGHT_BIT = 1; // 默认，截取的Ipv6字符串右结束位

    private int ipv6AddrNum = 0; // 记录Ipv6地址的个数
    private String ipv6AddrString = ""; // 由Ipv6地址组成的个数
    private List<String> ipv6AddrStringList = new ArrayList<>(); // 由Ipv6地址组成的个数
    ...
    public List<String> getIpv6AddrStringList() {
        return ipv6AddrStringList;
    }
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                if ((line.contains("inet6")) && (line.contains("scope"))) {
                    ...
                    ipv6AddrStringList.add(ipv6addrTemp);
